	   ...
	   <div class="course-card layout row wrap">
			...
			>
				...
				<app-card customClasses="course-hover-item">
					<div class="header">
						<div class="meta-info">
							<span class="date-info fs-12">Last updated: {{list.lastUpdates}}</span>
						</div>
						<div class="card-header"><h4><a href="#">{{list.name}}</a></h4></div>
						<div class="meta-info mb-1">
							<div class="mb-1" v-if = "list.bestseller == true">
								<span class="category fs-12"><span class="bestseller-tag">bestseller</span> {{list.bestSell}}</span>
							</div>
							<span class="meta-info-block">                                             
								<span class="lectures fs-12"><v-icon class="cmr-8">play_circle_filled</v-icon>{{list.lectures}} lectures</span>
								<span class="durations fs-12"><v-icon class="cmr-8">access_time</v-icon>{{list.hours}} hours</span>
								<span class="durations fs-12"><v-icon class="cmr-8">show_chart</v-icon>{{list.level}} Levels</span>
							</span>
						</div>
					</div>
